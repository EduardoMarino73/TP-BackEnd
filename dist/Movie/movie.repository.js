...
const toMovie = (row) => new Movie(row.id_author, row.path, row.title, row.category, Number(row.views), row.description, Boolean(row.state), undefined, row.id);
export class MovieRepository {
    async findAll() {
        ...
        const rows = result[0]*/
        const [rows] = await db.query("SELECT id, id_author, path, title, category, views, description, state FROM movies ORDER BY id");
        /*map apply a callback function to every item into the array rows*/
        return rows.map(toMovie);
    }
    async findOne(id) {
        /*take the firts parameter*/
        const [rows] = await db.query("SELECT id, id_author, path, title, category, views, description, state FROM movies WHERE id = ?", [id]);
        /*ternary operators
        try to return a movie or, insted return undefined */
        return rows[0] ? toMovie(rows[0]) : undefined;
    }
    async create(item) {
        const [result] = await db.execute("INSERT INTO movies (id_author, path, title, category, views, description, state) VALUES (?, ?, ?, ?, ?, ?, ?)", [item.id_author, item.path, item.title, item.category, item.views, item.description, item.state]);
        /**it set the id of the movie that inserted into the table of the database*/
        item.id = result.insertId;
        return item;
    }
    async update(id, input) {
        const allowedFields = ["id_author", "path", "title", "category", "views", "description", "state"];
